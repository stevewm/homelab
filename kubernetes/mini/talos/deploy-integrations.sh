...
rm -rf cni/charts
envsubst < $K8S_ROOT/core/kube-system/cilium/app/values.yaml > cni/values.yaml
kustomize build --enable-helm cni | kubectl apply -f -
rm cni/values.yaml
rm -rf cni/charts

rm -rf kubelet-csr-approver/charts
envsubst < $K8S_ROOT/core/system/kubelet-csr-approver/app/values.yaml > kubelet-csr-approver/values.yaml
